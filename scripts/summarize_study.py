...
def main():
    ...
    with StorageBudget(args.root).reserve(files=16*(1<<20),heap=2*GIB,purpose='paired study analysis'):
        ...
        for row in collected['records']:
            ...
            path=directory/name/'validation'/(Path(record['checkpoint']).stem+'-'+record['sha256'][:12]+'-metrics.npz')
            ...
            evidence[name]=dict(**arrays,dataset_id=record['dataset_id'])
            ...
            cost=(cnn_cost(channels=config['channels'],blocks=config['blocks']) if family=='cnn' else
                  fly_cost(graph['neurons'],graph['edges'],graph['sensory_neurons'],passes=config['steps'],
                           groups=config['groups'],readout_neurons=readout))
            panels={}
            ...
            records.append(dict(run=name,family=family,seed=config['seed'],model=config,
                checkpoint_sha256=record['sha256'],metrics_sha256=sha256(path),dataset_id=record['dataset_id'],
                optimizer_updates=record['optimizer_step'],batch_size=plan['batch_size'],
                training_position_exposures=record['optimizer_step']*plan['batch_size'],
                validation=record['slices'],inference_cost=cost,matches=panels))
        contrast=None
        if args.pair:
            deltas=[];pairs=[];reference=None
            ...
            contrast=dict(direction='candidate minus reference',pairs=pairs,
                ...
                uncertainty='Game bootstrap conditions on these trained checkpoints. Sample SD describes variation across paired training seeds; three seeds do not establish a precise seed-population confidence interval.')
        result=dict(status='complete' if complete else 'partial',study=args.run_id,records=records,contrast=contrast,
            collected_sha256=sha256(directory/'summary.json'),plan_sha256=sha256(directory/'plan.json'),
