...
def forward(params, graph, ports, features, *, steps, groups, actions):
    ...
    weight = graph['sign'][source] * jax.nn.softplus(params['edge'])
    alpha = 0.01 + 0.98 * jax.nn.sigmoid(params['leak'][graph['type_id']])
    ...
    logits = pooled.T @ params['policy_weight'].reshape(actions,groups).T + params['policy_bias']
    value = jnp.tanh(pooled.T @ params['value_weight'] + params['value_bias'][0])
...
def loss(params,graph,ports,features,legal,policy,value,*,steps,groups,actions):
    ...
    policy_loss=-(policy*jax.nn.log_softmax(logits)).sum(axis=-1).mean()
...
def adam(params,grad,first,second,step,*,rate=0.003,clip=1.0):
    ...
    norm=jnp.sqrt(sum(jnp.sum(jnp.square(g.astype(jnp.float64))) for g in grad.values()))
    ...
    params={k:params[k]-rate*(first[k]/c1)/(jnp.sqrt(second[k]/c2)+1e-8) for k in params}
