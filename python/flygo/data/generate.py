...
from ..storage import GIB, StorageBudget, StoragePressure, _process_identity
...
def play_game(teacher, opponent, contract, game_id, expert_color):
    ...
    for ply in range(contract['max_moves']):
        ...
        teacher_future = teacher.submit(query(actions, visits=teacher_visits))
        behavior_future = teacher_future if teacher_turn else opponent.submit(query(actions, visits=contract['visits']))
        legal_actions = game.legal()
        label = parse_label(teacher_future.result(timeout=180), legal_actions, state.to_play)
        behavior = label if teacher_turn else parse_label(behavior_future.result(timeout=180), legal_actions, state.to_play)
        ...
        if ply < contract['opening_moves']:
            ...
            action = int(rng.choice(82, p=distribution))
        else:
...
def run_worker(config_path: Path, worker_index: int, *, batches: int | None = None):
    ...
    with budget.reserve(files=32 * 1024**2, heap=8 * GIB, purpose=f'data worker {config["run_id"]}/{worker_index}'):
        with ExitStack() as stack:
            teacher = stack.enter_context(AnalysisClient(binary, model(contract['teacher']), output / 'teacher',
                    cpus, concurrency=concurrent_games, eigen_threads=4, max_pending=concurrent_games * 2))
            opponent = stack.enter_context(AnalysisClient(binary, model(opponent_record), output / 'opponent',
                    cpus, concurrency=concurrent_games, eigen_threads=2, max_pending=concurrent_games * 2))
