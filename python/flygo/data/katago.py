...
def config_text(*, concurrency=4, eigen_threads=4, seed='flygo-data-v1') -> str:
    values = dict(numAnalysisThreads=concurrency, numSearchThreadsPerAnalysisThread=1,
                  ...
                  nnMutexPoolSizePowerOfTwo=12, maxBoardXSizeForNNBuffer=9,
                  maxBoardYSizeForNNBuffer=9, requireMaxBoardSize=True,
...
class AnalysisClient:
    def __init__(self, binary: Path, model: Path, output: Path, cpus, *, concurrency=4,
                 eigen_threads=4, max_pending=16, seed='flygo-data-v1'):
        output.mkdir(parents=True, exist_ok=True)
        config = output / 'analysis.cfg'
        config.write_text(config_text(concurrency=concurrency, eigen_threads=eigen_threads, seed=seed))
