...
def split_for_family(family):
    number=int(family[:8],16)%100
    return 'test' if number<5 else 'validation' if number<10 else 'train'
...
def publish_game(directory: Path, metadata: dict, rows: list[dict], actions: list[int]) -> Path:
    ...
    family = opening_family(actions)
    metadata = {**metadata, 'schema_version': 1, 'rows': len(rows), 'opening_family': family,
...
def audit_game(path: Path) -> dict:
    with np.load(path, allow_pickle=False) as archive:
        # NpzFile re-decompresses a column on each access. Load this bounded game
        # once before checking its rows; validation semantics remain identical.
        ...
        if opening_family(actions.tolist()) != meta['opening_family']:
            raise ValueError('Incorrect split family')
        if split_for_family(meta['opening_family']) != meta['split']:
            raise ValueError('Incorrect family split assignment')
        return meta
