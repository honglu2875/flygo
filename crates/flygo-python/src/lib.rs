...
fn _native(module: &Bound<'_, PyModule>) -> PyResult<()> {
    module.add("ABI_VERSION", 1)?;
    module.add_function(wrap_pyfunction!(observations::replay_observations, module)?)?;
    module.add_function(wrap_pyfunction!(observations::replay_features, module)?)?;
    module.add_function(wrap_pyfunction!(
        observations::replay_suffix_observations,
        module
    )?)?;
    module.add_class::<Actors>()?;
    module.add_class::<game::Game>()?;
    module.add_class::<fly::FlyModel>()?;
    Ok(())
}
