...
impl Game {
    ...
    /// Encode the current board without starting a search or changing state.
    fn features<'py>(&self, py: Python<'py>) -> PyResult<Bound<'py, PyArray1<f32>>> {
        let features = py
            .detach(|| {
                let game = self.inner.lock().map_err(|e| e.to_string())?;
                Ok::<_, String>(game.position.features(&game.legal()))
            })
            .map_err(PyValueError::new_err)?;
        Ok(features.into_pyarray(py))
    }
